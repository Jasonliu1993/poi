package com.jwebcoder.poi.poisax;
...
public class Excel2007Reader extends DefaultHandler {
    ...
    private List<Cell> rowlist = new ArrayList<Cell>();
    //当前行
...
//        System.out.println("startElement++++++++++++++localName: " + localName + " name: " + name + " attributes:" + attributes.getValue("s"));
        // c => 单元格
        if ("c".equals(name)) {

            //获取当前行
            String excelColumnNum = attributes.getValue("r");
            currentRow = getCurRow(excelColumnNum);
            currentColumn = getCurRow(excelColumnNum);
...
//        System.out.println("endElement++++++++++++++localName: " + localName + " name: " + name);
        ...
        if (isTElement) {
            ...
            rowlist.add(curCol, new Cell(CellDataType.isString, value));
            curCol++;
            isTElement = false;
    ...
    public int getCurRow(String excelColumnNum){
        //Pattern是一个正则表达式经编译后的表现模式
        Pattern p2 = Pattern.compile(NUMBER_REGEX);
        Matcher m2 = NUMBER_PATTERN.matcher(excelColumnNum);
        if(m2.find()){
